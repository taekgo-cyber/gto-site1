        ...
        <CardContent>
          <form method="get" className="flex flex-wrap items-end gap-3">
            <label className="min-w-56 flex-1 text-sm font-medium">현재 기업
              <select name="companyId" defaultValue={selectedCompanyId} className="mt-1 h-10 w-full rounded-md border border-border bg-background px-3 text-sm">
      ...
      <Card>
        ...
        <CardContent>
          <form method="get" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <select name="preferredRegionId" defaultValue={query.filters.preferredRegionId ?? ""} className="h-10 rounded-md border border-border bg-background px-3 text-sm"><option value="">희망 지역 전체</option>{regions.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select>
            <select name="vehicleTypeId" defaultValue={query.filters.vehicleTypeId ?? ""} className="h-10 rounded-md border border-border bg-background px-3 text-sm"><option value="">차종 전체</option>{vehicleTypes.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select>
            <select name="tonnageId" defaultValue={query.filters.tonnageId ?? ""} className="h-10 rounded-md border border-border bg-background px-3 text-sm"><option value="">톤수 전체</option>{tonnages.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select>
            <select name="desiredWorkType" defaultValue={query.filters.desiredWorkType ?? ""} className="h-10 rounded-md border border-border bg-background px-3 text-sm"><option value="">근무 형태 전체</option><option value="FULL_TIME">전일제</option><option value="PART_TIME">파트타임</option><option value="CONTRACT">계약직</option><option value="DAILY">일용직</option><option value="FREELANCE">프리랜서</option></select>
            <input name="minExperienceYears" type="number" min="0" placeholder="최소 경력(년)" defaultValue={query.filters.minExperienceYears ?? ""} className="h-10 rounded-md border border-border bg-background px-3 text-sm" />
            <select name="leaseExperience" defaultValue={query.filters.leaseExperience == null ? "" : String(query.filters.leaseExperience)} className="h-10 rounded-md border border-border bg-background px-3 text-sm"><option value="">지입 경험 전체</option><option value="true">있음</option><option value="false">없음</option></select>
            <select name="vehicleOwned" defaultValue={query.filters.vehicleOwned == null ? "" : String(query.filters.vehicleOwned)} className="h-10 rounded-md border border-border bg-background px-3 text-sm"><option value="">차량 보유 전체</option><option value="true">보유</option><option value="false">미보유</option></select>
