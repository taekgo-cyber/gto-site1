...
export default async function SupportPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  return (
    <Container className="mx-auto max-w-3xl space-y-6 py-8">
      ...
      <Card><CardHeader><CardTitle>문의 접수</CardTitle></CardHeader><CardContent><SupportForm defaults={{ name: user?.name, email: user?.email, phone: user?.phone ?? undefined, category: params.category }} /></CardContent></Card>
    </Container>
  );
}
