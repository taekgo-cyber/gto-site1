      ...
      <div className="mx-auto flex max-w-6xl flex-wrap gap-2 px-4 py-3 sm:px-6 lg:px-8">
        {campaigns.map((campaign) => {
          ...
          return campaign.linkUrl ? (
            <a key={campaign.id} href={`/api/ads/${encodeURIComponent(campaign.id)}/click`} rel="sponsored noopener noreferrer" className="flex min-w-56 flex-1 items-center gap-2 rounded-md border border-border bg-background px-4 py-3 text-sm transition-colors hover:bg-surface">{content}</a>
          ) : (
