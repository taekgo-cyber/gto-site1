...
export function PagedAdvertisementSection({ id, tier, heading, pages }: {
  ...
  useEffect(() => {
    // Next's back navigation can preserve state while reconnecting effects.
    const pager = createAdvertisementPager(pages.length, next => { pageRef.current = next; setPage(next); }, pageRef.current);
    controller.current = pager;
    pager.pause("user", stoppedRef.current);
    pager.pause("hover", sectionRef.current?.matches(":hover") ?? false);
    pager.pause("focus", sectionRef.current?.contains(document.activeElement) ?? false);
  ...
  return <section ref={sectionRef} className={`home-panel home-paged-section home-${tier.toLowerCase()}-section`} aria-labelledby={id} aria-roledescription="carousel" data-tier={tier} data-page-index={currentPage}
    ...
    {pages.length > 1 && <div className="home-page-controls" aria-label={`${tier} 페이지 탐색`}>
      ...
      <button type="button" disabled={reducedMotion} aria-pressed={stopped || reducedMotion} onClick={() => { const next = !stopped; stoppedRef.current = next; setStopped(next); controller.current?.pause("user", next); }}>{reducedMotion ? "자동순환 꺼짐" : stopped ? "자동순환 재생" : "자동순환 정지"}</button>
    </div>}
